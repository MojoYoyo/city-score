...
import { formatFoodDiningCounts, getFoodDiningIcon } from '../../../services/utils/formatters';

/**
 * Food & Dining score component
 */
const FoodDiningScore = ({ score }) => {
  if (!score) {
    return <p>Food & Dining score not available.</p>;
  }

  const formattedCounts = formatFoodDiningCounts(score.breakdown.counts);
      ...
      {formattedCounts.length > 0 ? (
        <div>
          {formattedCounts.map((item, index) => (
            <div key={index} className="food-dining-type">
              <span style={{ display: 'flex', alignItems: 'center' }}>
                <span style={{ marginRight: '6px' }}>{getFoodDiningIcon(item.type)}</span>
        ...
        <p>No food & dining facilities found within 1km.</p>
      )}
          ...
          {Object.entries(score.breakdown.byType).map(([type, value]) => {
            ...
            return (
              <li key={type} style={{ 
                ...
                <span style={{ display: 'flex', alignItems: 'center' }}>
                  <span style={{ marginRight: '6px' }}>{getFoodDiningIcon(typeName)}</span>
                  {typeName}
                </span>
                <span>
...
export default FoodDiningScore;