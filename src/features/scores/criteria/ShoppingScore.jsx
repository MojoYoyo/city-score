...
import { formatShoppingCounts, getShoppingIcon } from '../../../services/utils/formatters';

/**
 * Shopping score component
 */
const ShoppingScore = ({ score }) => {
  if (!score) {
    return <p>Shopping score not available.</p>;
  }

  const formattedCounts = formatShoppingCounts(score.breakdown.counts);
      ...
      {formattedCounts.length > 0 ? (
        <div>
          {formattedCounts.map((item, index) => (
            <div key={index} className="shopping-type">
              <span style={{ display: 'flex', alignItems: 'center' }}>
                <span style={{ marginRight: '6px' }}>{getShoppingIcon(item.type)}</span>
        ...
        <p>No shopping facilities found within 1km.</p>
      )}
          ...
          {Object.entries(score.breakdown.byType).map(([type, value]) => {
            ...
            return (
              <li key={type} style={{ 
                ...
                <span style={{ display: 'flex', alignItems: 'center' }}>
                  <span style={{ marginRight: '6px' }}>{getShoppingIcon(typeName)}</span>
                  {typeName}
                </span>
                <span>
