...
import { formatRecreationSportsCounts, getRecreationSportsIcon } from '../../../services/utils/formatters';

/**
 * Recreation & Sports score component
 */
const RecreationSportsScore = ({ score }) => {
  if (!score) {
    return <p>Recreation & Sports score not available.</p>;
  }

  const formattedCounts = formatRecreationSportsCounts(score.breakdown.counts);
      ...
      {formattedCounts.length > 0 ? (
        <div>
          {formattedCounts.map((item, index) => (
            <div key={index} className="recreation-sports-type">
              <span style={{ display: 'flex', alignItems: 'center' }}>
                <span style={{ marginRight: '6px' }}>{getRecreationSportsIcon(item.type)}</span>
        ...
        <p>No recreation & sports facilities found within 1km.</p>
      )}
          ...
          {Object.entries(score.breakdown.byType).map(([type, value]) => {
            ...
            return (
              <li key={type} style={{ 
                ...
                <span style={{ display: 'flex', alignItems: 'center' }}>
                  <span style={{ marginRight: '6px' }}>{getRecreationSportsIcon(typeName)}</span>
                  {typeName}
                </span>
                <span>
...
export default RecreationSportsScore;