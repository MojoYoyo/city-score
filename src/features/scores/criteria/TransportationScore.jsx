...
import { formatTransportationCounts } from '../../../services/utils/formatters';
import { getTransportIcon } from '../../../services/utils/formatters';

/**
 * Transportation score component
 */
const TransportationScore = ({ score }) => {
  if (!score) {
    return <p>Transportation score not available.</p>;
  }

  const formattedCounts = formatTransportationCounts(score.breakdown.counts);
      ...
      {formattedCounts.length > 0 ? (
        <div>
          {formattedCounts.map((item, index) => (
            <div key={index} className="transportation-type">
              <span style={{ display: 'flex', alignItems: 'center' }}>
                <span style={{ marginRight: '6px' }}>{getTransportIcon(item.type)}</span>
        ...
        <p>No transportation stops found within 1km.</p>
      )}
          ...
          {Object.entries(score.breakdown.byType).map(([type, value]) => {
            ...
            return (
              <li key={type} style={{ 
                ...
                <span style={{ display: 'flex', alignItems: 'center' }}>
                  <span style={{ marginRight: '6px' }}>{getTransportIcon(typeName)}</span>
                  {typeName} 
                  {type === 'metro' && <span style={{color: '#C60C30', marginLeft: '4px'}}>(Premium)</span>}
                </span>
                <span>
...
export default TransportationScore;